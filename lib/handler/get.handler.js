...
let GetHandler = module.exports = Handler.extend({
...
GetHandler.prototype.do = function () {
	GetHandler.__super__.do();
	const stateMap = {
		'open': '待办中',
		'progressing': '进行中',
		'closed': '关闭',
...
GetHandler.prototype.getMilestone = function () {
	let milestoneUrl = `https://gitee.com/api/v5/repos/bc-mall/mall-issue/milestones?access_token=${this.options.gtoken}&state=open&sort=due_on&page=1&per_page=100`;
	return new Promise((resolve, reject) => {
		request(milestoneUrl, (error, response, body) => {
			...
			for (let milestone of milestoneArray) {
				if (milestone.title.indexOf('产品-') != -1) {
...
GetHandler.prototype.getIssues = function (milestone) {
	let milestoneUrl = `https://gitee.com/api/v5/repos/bc-mall/mall-issue/issues?access_token=${this.options.gtoken}&state=all&sort=created&direction=desc&page=1&per_page=100&milestone=${encodeURIComponent(milestone.title)}`;
	return new Promise((resolve, reject) => {
		request(milestoneUrl, (error, response, body) => {
			if (error) reject(error);
			if (response.statusCode != 200) {
...
GetHandler.prototype.sendMsg = function (milestone, fileName) {
