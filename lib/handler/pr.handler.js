...
PrHandler.prototype.do = function () {
	...
	PrHandler.__super__.getGiteeToken().then((token)=>{
		this.options.gtoken = token;
		this.getProject().then((trs) => {
			if (trs) {
				let html = htmlString.replace('#{tr}', trs);
				(async () => {
					let fileName = `${new Date().getTime()}-pr.png`;
					const browser = await puppeteer.launch()
					const page = await browser.newPage()
					await page.setContent(html)
					await page.screenshot({ path: fileName, fullPage: true })
					await browser.close();
					this.sendMsg("Pull Request", fileName)
				})();
			}
		})
	})
...
PrHandler.prototype.getProject = function () {
	return new Promise((resolve, reject) => {
		...
		for (let project of projectArray) {
			this.getPR(project).then((prArray) => {
				...
				if (prArray.length) {
					prArray.sort((a, b) => {
						if (a.base.repo.name > b.base.repo.name) {
							return 1;
						} else if (a.base.repo.name < b.base.repo.name) {
							return -1;
						}
						return 0;
					})
...
PrHandler.prototype.getPR = function (project) {
	let milestoneUrl = `https://gitee.com/api/v5/repos/${this.options.repo}/${project}/pulls?access_token=${this.options.gtoken}&state=open&sort=created&direction=desc&page=1&per_page=100`;
	return new Promise((resolve, reject) => {
